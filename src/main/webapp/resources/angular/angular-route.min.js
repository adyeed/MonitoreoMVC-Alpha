	...
	p = c.module ( "ngRoute" , [ "ng" ] ).provider ( "$route" , function () {
		...
		this.$get = [ "$rootScope" , "$location" , "$routeParams" , "$q" , "$injector" , "$templateRequest" , "$sce" , function ( a , f , b , d , h , p , x ) {
			...
			var w = ! 1 , n , v , s = {
				...
				updateParams : function ( a ) {if ( this.current && this.current.$$route )a = c.extend ( {} , this.current.params , a ), f.path ( t ( this.current.$$route.originalPath , a ) ), f.search ( a ); else throw B ( "norout" );}
			};
